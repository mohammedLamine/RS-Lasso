...
class RSLasso : 
    ...
    def train(self,split_t=13):
        valid_split=0.67
        self.split=split_t
        
        left_speed_df,right_speed_df =  RSLasso.split_data(self.train_data,13)[True],RSLasso.split_data(self.train_data,13)[False]
        left_speed_test_df,right_speed_test_df =  RSLasso.split_data(self.test_data,13)[True], RSLasso.split_data(self.test_data,13)[False]
    ...
    def predict(self):
        ...
        print("MSE:", mean_squared_error(self.test_prediction.values.flatten(), self.test_data[self.test_prediction.columns].values.flatten()))
        print("MAE:", mean_absolute_error(self.test_prediction.values.flatten(), self.test_data[self.test_prediction.columns].values.flatten()))
from geolytics_analysis import paper_models as pm
import pandas as pd
tv_data = pd.read_pickle("../data/train_validation_data.pckl")
ts_data = pd.read_pickle("../data/test_data.pckl")
from geolytics_analysis import paper_models as pm
lasso = pm.RSLasso(tv_data[:20],ts_data[:20])
lasso.train()
lasso.predict()
